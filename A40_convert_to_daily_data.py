...
def convert_to_daily(args):
    ...
    # 3.) Drop untreated patients - not sure if this is a good idea, but I think those patients' data are probably right-censored
    valid_df = clean_df[(clean_df.start_epoch.notnull()) & (clean_df.end_epoch.notnull()) & (clean_df["Type of VOE"]=='VOC') & (clean_df.patient_treated)]

    # Make sure data is sorted by patient and then start_epoch
    valid_df = valid_df.sort_values(by=['Unique Subject Identifier','start_epoch'])
    
    min_date = pd.to_datetime(min(valid_df["Infusion Date"].min(), valid_df["Onset date"].min())) # Estimated earliest date we need
    ...
    for i in pd.unique(valid_df["Unique Subject Identifier"]):
        ...
        for _, row in temp_df.iterrows():
            if first:
                first = False # N.B. We skip the first episode to conservatively bias the proportion of time
                              # in hospital before treatment down (and to compensate for the left-censoring of the data).
        ...
        temp_copy = daily_df.copy(deep=True) # To deal with the fact that I want to iterate over daily_df while changing it
        for day, row in temp_copy.iterrows():
            if (str(day)[:10]) <= earliest_resolution_date:
                continue
            before = (str(day)[:10] < infusion)
            if before:
                daily_df.loc[day][suffix+"_OOH_Before"] = 1 - daily_df.loc[day][suffix+"_IH_Before"]
            else: # We don't see this case in our data, but for completeness' sake.
                daily_df.loc[day][suffix+"_OOH_After"] = 1 - daily_df.loc[day][suffix+"_IH_After"]
                
    # We only estimated the start date before.  This next bit lops off the top of the dataframe that we don't need.            
    daily_df["row_sum"] = daily_df.sum(axis=1)
    daily_df = daily_df[daily_df.row_sum>0] # We only estimated the start date before.  This lops off the top of the dataframe that we don't need.
    daily_df.drop(labels="row_sum", axis=1, inplace=True)
    
    # To calculate proportions for SPRT later, it will be nice to have cumulative sums before and after the treatment per patient.
    for i in pd.unique(valid_df["Unique Subject Identifier"]):
        suffix = i[-4:]
